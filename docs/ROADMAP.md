...
### 6.4 Frame-rate cap (user-requested; all platforms) — IMPLEMENTED
Cap video-wallpaper output frame rate to trade smoothness for power/heat — the ask from low-end hardware (e.g. Intel N150): a 60fps clip pinned to 30 or 24 halves present/compositor work and vsync wakeups, a real battery win, and some users prefer 24 for a "cinematic" cadence. Shipped at **two levels**: a **global** `framerate: u16` on `Config` (0 = original rate) mirroring `scaling` in the "Video quality" settings group — the machine-wide battery knob — plus a **per-wallpaper** override `Wallpaper.framerate: Option<u16>` (`None` inherits the global) exposed in the crop/rotate editor, so a single cinematic clip can run at 24 while everything else stays smooth. `Wallpaper::effective_framerate(global)` resolves the two. Mechanism is small because crop (`video-zoom`/`video-pan`) and rotation (`video-rotate`) use mpv **properties**, leaving the `vf` chain free: `config::fps_filter(fps)` yields the libavfilter `fps=N` filter as the sole `vf` occupant, applied at spawn on both X11/libmpv (`set_option`) and Wayland/mpvpaper (`-o vf=…`), and re-applied live via `set_framerate` (property write) on scheduled swaps so a per-wallpaper cap never leaks onto the next wallpaper. Settings/editor dropdowns save and call `ensure_daemon_and_apply`, so renderers respawn immediately with the daemon staying up. **Honesty line:** the `fps` filter drops frames *after* decode, so it cuts render/present/vsync load (the battery and heat win) but not decode work — never advertise it as reducing decode. GNOME-static fallback has no live video, so the cap is inert there.
- **Done:** `fps_filter` (0→None, N→"fps=N") and `effective_framerate` (override ?? global) unit-tested; `Config.framerate` + `Wallpaper.framerate` + `LibraryEntry.framerate` round-trip TOML/JSON with absent-key defaults; both backends thread the effective value through spawn and a `set_framerate` runtime setter; `WlOutput::apply_wallpaper` treats a framerate change as respawn-forcing; scheduled swaps re-apply it; global Settings ComboRow + per-wallpaper editor ComboRow ("Default" = inherit) wired. 77 daemon tests + clippy/fmt green.
- **Pending on-screen check:** confirm ~30fps output and lower GPU present/power on a real display (VM is llvmpipe/no-GPU); the `vf=fps` mechanism itself is standard mpv.
