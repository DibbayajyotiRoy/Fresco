...
-- ── Check-in history (install_days) ───────────────────────────────────────
-- One row per install per calendar day it checked in. `installs` is upserted
-- in place (first_seen/last_seen are overwritten every heartbeat), so it has
-- no per-day history and cannot answer "how many installs checked in today"
-- or feed a retention cohort. This table is the smallest addition that does:
-- written by the same SECURITY DEFINER functions that already write
-- `installs`, so a heartbeat that already reaches the server costs nothing
-- new to send. Also committed standalone as
-- supabase/migrations/2026-09-24_install_days.sql — see admin/README.md.
--
-- Idempotent: safe to run on any state, including a project that already has
-- `installs`/`register_install` from earlier in this file.
