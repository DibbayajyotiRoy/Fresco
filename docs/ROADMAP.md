...
### 6.4 Power saving (user-requested; all platforms) — IMPLEMENTED, mechanism corrected TWICE
Cut GPU load for a video wallpaper on weak hardware (Intel N150). Shipped at two levels: global `power_saving: PowerSaving` on `Config` (Full / Reduced / Minimum) plus a per-wallpaper `Wallpaper.power_saving: Option<PowerSaving>` override, resolved by `effective_power_saving`.

**Two wrong mechanisms shipped before the right one — the standing lesson of this item is _measure on real hardware before claiming a perf win_:**
1. **1.1.32 `fps` filter** — a *software* filter in a VA-API pipeline forces every frame off the GPU; the reporter measured video-engine load ~doubling (17%→34%). Actively harmful.
