...
### 6.4 Power saving (user-requested; all platforms) — IMPLEMENTED, mechanism corrected
Cut video-decode load to save battery and heat on weak hardware (the ask from an Intel N150 user). Shipped at **two levels**: a global `power_saving: PowerSaving` on `Config` (Full / Reduced / Minimum) in the "Video quality" settings group, plus a per-wallpaper override `Wallpaper.power_saving: Option<PowerSaving>` (`None` inherits) in the crop/rotate editor, resolved by `Wallpaper::effective_power_saving(global)`.

**The first attempt was wrong and shipped in 1.1.32.** It capped output fps with an `fps` video filter. A video filter is software, so inserting it into a VA-API pipeline forces every frame off the GPU: the reporter measured video-engine load roughly doubling (~17% -> ~34%) when capping 60fps to 30 — the exact opposite of the goal, on the exact hardware it targeted. Lesson recorded here deliberately: **a "perf" feature that was never measured on real hardware is a guess.**

The corrected mechanism is decoder-level skipping via `--vd-lavc-skipframe` (Reduced -> `nonref`, Minimum -> `bidir`, Full -> untouched), which discards frames inside libavcodec *before* decode, leaving hardware decoding intact. Applied at spawn on both backends and re-applied live via `set_power_saving` on scheduled swaps. It cannot express an exact fps — savings depend on GOP structure — so the control is named for what it does.
- **Verified before release** with `cargo run --example skipframe_probe <video>` (committed): on `vaapi-copy` and `nvdec-copy`, Reduced cut frames reaching the output ~47% and Minimum ~73%, with `hwdec-current` still reporting a hardware decoder in every run.
- **Done:** `PowerSaving::skipframe()` and `effective_power_saving` unit-tested; legacy 1.1.32 `framerate` keys parse and migrate to `Reduced` via `Config::migrate` (tested, global and per-wallpaper); both backends carry the level through spawn + runtime setter; Settings and editor ComboRows wired. 79 tests + clippy/fmt green.
- **Still open:** confirm the *power* win end-to-end with `intel_gpu_top` on the reporter's box (fewer decoded frames is proven; watts is not). Requires perf access this host does not have unprivileged.
